...
int main(){
    ...
    for(int i = 0; i < 20; ++i){
        ...
        double Ex_B = (data_Extraction_Value(fileEx, 251, B_11_check))/(1000); //determines the excitation energy
        m_B_11 = m_B_11 + Ex_B; //change the mass of 11B
        B_11_check = 0;

        //Q-value and max energy of electron
        double Q = m_Be_11 - Ex_B; //Set the resulting Q-value

        //electron energy and momentum
        electron_energy(electron, Q); //determines the max kinetic energy
        electron.p[0] = electron.p[0] + me; //total energy
